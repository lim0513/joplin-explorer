...
**空分区常驻与拖拽增强**（v1.6.6）
- 收藏夹 / 标签 / 回收站分区**空了也不再消失**：标题常驻（计数为零时隐藏徽标），与堆叠标题"分区随时可达"的承诺一致；回收站与 Joplin 原生侧栏行为对齐——[#38](https://github.com/lim0513/joplin-explorer/issues/38)，感谢 [@CJeffyB](https://github.com/CJeffyB)。收藏夹标题常驻后，#13 的临时拖拽落点注入方案退役
- **拖拽悬停自动展开**：拖拽悬停折叠的文件夹（拖入区）、收藏夹标题、笔记本/标签分区头 2 秒自动展开，可逐层深入
- **收藏夹任意位置拖放**：拖任意笔记/笔记本到收藏夹内即收藏，落点即位置（悬停收藏项上/下半插入其前/后，空白处追加）；已收藏项拖入等于调位

**新建笔记本对话框修复**（v1.6.7）
- 新版 Joplin 的 `openFolderDialog` 要求必须传 `parentId`（根级为空串），否则静默失败：工具栏"新建笔记本"与拖拽到底部落区建本因此无响应，现已修复；拖拽建本也换用原生对话框（可选图标），建完自动把拖入条目移进新笔记本

**标签变化自动刷新**（v1.6.8）
- 给当前笔记打/去标签、以及在任意处新建/重命名/删除标签，面板标签区在一个自动刷新周期（5 秒）内自动跟上，无需手动刷新。实现为两个轻量指纹（当前笔记标签集 + 标签表），挂在既有 autoRefresh 轮询上；Joplin 的事件接口不含标签变更，故只能指纹轮询
