...
- **How a fit-to-content dialog is sized** (read from Joplin 3.x app.asar, verified by replaying it): the webview measures `getBoundingClientRect()` of its own block-level `#joplin-plugin-content` and sets the iframe to that. A block is as wide as the iframe already is (~300px at first), so a wider child NEVER grows the dialog - with `max-width:100%` it just gets squeezed to ~300px. The small confirm/input dialogs above are fine with that; anything wider must add `#joplin-plugin-content { width: max-content; }` in its own stylesheet and give its root a fixed width WITHOUT `max-width:100%` (see `webview/smart-dialog.css`). The smart folder manager first shipped to testing at ~300px with a sideways-scrolling list because of this.
- Joplin's dialog script submits the form (= OK) on Enter in any `<input type="text">`, from its own document keydown listener. Blocking the form's `submit` event does not stop it.
